    ...
    public class BaseRepository<Entity> : IBaseRepository<Entity>
    {
        ...
        public object GetById(Guid entityId)
        {
            using (_dbConnection = new MySqlConnection(_connectionString))
            {
                ...
                if (className == "Employee")
                {
                    sqlCommand = $"SELECT e.*, d.DepartmentName, (CASE e.Gender WHEN 0 THEN 'Nữ' WHEN 1 THEN 'Nam' ELSE 'Không xác định' END) AS GenderName FROM {className} e LEFT JOIN Department d ON d.DepartmentId = e.DepartmentId WHERE {$"{className}Id"} = @{className}IdParam ORDER BY e.CreatedDate DESC";
                }
                if (className == "Customer")
                {
